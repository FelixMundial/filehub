...
//            Map<String, Object> token = new HashMap<>(requestParameters);
            ...
            principal.put("uid", ((SecurityUser) authentication.getPrincipal()).getCurrentUser().getUserId());
            principal.put("username", ((SecurityUser) authentication.getPrincipal()).getCurrentUser().getUserLoginName());
            principal.put("nickname", ((SecurityUser) authentication.getPrincipal()).getCurrentUser().getUserNickname());
...
//            token.put("token", oAuth2AuthenticationDetails.getTokenValue());
        ...
        BaseResult successResult = BaseResultFactory.getSuccessResult(String.format("%s用户登录成功", authentication.getName()), dataMap);
